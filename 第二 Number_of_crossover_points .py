...
for idx in range(GEN):#try small range at the beginning to see the diagram is correct
    ...
    for i in range (0, P):
        ...
        tempoff = [off1, off2, off3] 
        #print('First',off1.fitness)
        bestone = min(tempoff, key=lambda x: x.fitness)#A lambda function can take any number of arguments, but can only have one expression.
        #print('Best',bestone.fitness)
        offspring.append( bestone )

    toff1 = individual()
    toff2 = individual()
    temp = individual()
    #方法一Two-Point Crossover_1
    # for i in range(0, P, 2):
    #     toff1 = copy.deepcopy(offspring[i])
    #     toff2 = copy.deepcopy(offspring[i+1])
    #     temp = copy.deepcopy(offspring[i])
    #     crosspoint = random.randint(1,N)
    #     crosspoint1 = random.randint(crosspoint,N)
    #     for j in range (1,crosspoint):
    #         toff1.gene[j] = toff2.gene[j]
    #         toff2.gene[j] = temp.gene[j]
    #     offspring[i] = copy.deepcopy(toff1)
    #     offspring[i+1] = copy.deepcopy(toff2)
    #     for k in range (crosspoint1,N):
    #         toff1.gene[k] = toff2.gene[k]
    #         toff2.gene[k] = temp.gene[k]
    #     offspring[i] = copy.deepcopy(toff1)
    #     offspring[i+1] = copy.deepcopy(toff2)
    #方法二Two-Point Crossover_2
    for i in range(0, P, 2):
        toff1 = copy.deepcopy(offspring[i])
        toff2 = copy.deepcopy(offspring[i+1])
        temp = copy.deepcopy(offspring[i])
        crosspoint = random.randint(1,N)
        crosspoint1 = random.randint(crosspoint,N)
        for j in range (crosspoint,crosspoint1):
            toff1.gene[j] = toff2.gene[j]
            toff2.gene[j] = temp.gene[j]
        offspring[i] = copy.deepcopy(toff1)
        offspring[i+1] = copy.deepcopy(toff2)
    #方法三 uniform crossover 均匀交叉
    # for i in range(0, P, 2):
    #     toff1 = copy.deepcopy(offspring[i])
    #     toff2 = copy.deepcopy(offspring[i+1])
    #     temp = copy.deepcopy(offspring[i])
    #     for j in range(len(toff1.gene)):
